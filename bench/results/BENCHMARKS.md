...
> **TL;DR**
>
> - Compress a **4.14 MB** JPEG in **170.1 ms** on Chrome 149 (`canvas-main` path).
> - **`canvas-main`** is the fastest on this hardware (170.1 ms).
...
**Run at:** 2026-09-13T10:25:39.244Z
...
| --- | --- | --- | --- | --- | --- |
| `full` | `webcodecs-worker` | 48.4 ms | 42.8 ms | 448.8 KB | 18.8% |
| `no-webcodecs` | `offscreen-worker` | 48.2 ms | 42.8 ms | 448.8 KB | 18.8% |
| `no-workers` | `canvas-main` | 35.0 ms | 34.3 ms | 448.8 KB | 18.8% |
...
<svg viewBox="0 0 730 104" xmlns="http://www.w3.org/2000/svg" font-family="ui-sans-serif, system-ui, sans-serif" font-size="12" role="img" aria-label="Benchmark: medium-1500x1000.jpg">
  ...
  <rect x="160" y="10" width="339.9" height="18" fill="#7aa2ff" rx="3" opacity="1"/>
  <text x="505.876033057841" y="23" fill="#5dd39e" font-weight="700">35.0 ms ⚡</text>
  <text x="152" y="52" text-anchor="end" fill="#cbd5ff" font-weight="500">offscreen-worker</text>
  <rect x="160" y="42" width="468.1" height="18" fill="#9c7cff" rx="3" opacity="0.85"/>
  <text x="634.0578512396978" y="55" fill="#cbd5ff" font-weight="500">48.2 ms</text>
...
| Config | Actual path | Time (median) | Time (best) | Output | Saved |
| --- | --- | --- | --- | --- | --- |
| `full` | `webcodecs-worker` | 182.3 ms | 180.8 ms | 414.2 KB | 90.2% |
| `no-webcodecs` | `offscreen-worker` | 183.3 ms | 181.8 ms | 414.2 KB | 90.2% |
| `no-workers` | `canvas-main` | 170.1 ms | 167.1 ms | 414.2 KB | 90.2% |
...
<svg viewBox="0 0 730 104" xmlns="http://www.w3.org/2000/svg" font-family="ui-sans-serif, system-ui, sans-serif" font-size="12" role="img" aria-label="Benchmark: large-4000x3000.jpg">
  ...
  <line x1="395.0" y1="4" x2="395.0" y2="100" stroke="#30363d" stroke-width="0.5" stroke-dasharray="2,2"/>
  <text x="395.0" y="103" text-anchor="middle" fill="#6e83b8" font-size="10">92 ms</text>
  <line x1="512.5" y1="4" x2="512.5" y2="100" stroke="#30363d" stroke-width="0.5" stroke-dasharray="2,2"/>
  <text x="512.5" y="103" text-anchor="middle" fill="#6e83b8" font-size="10">137 ms</text>
  <line x1="630.0" y1="4" x2="630.0" y2="100" stroke="#30363d" stroke-width="0.5" stroke-dasharray="2,2"/>
  <text x="630.0" y="103" text-anchor="middle" fill="#6e83b8" font-size="10">183 ms</text>
  <text x="152" y="20" text-anchor="end" fill="#f1f5ff" font-weight="700">canvas-main</text>
  <rect x="160" y="10" width="436.2" height="18" fill="#7aa2ff" rx="3" opacity="1"/>
  <text x="602.1538461538528" y="23" fill="#5dd39e" font-weight="700">170.1 ms ⚡</text>
  <text x="152" y="52" text-anchor="end" fill="#cbd5ff" font-weight="500">webcodecs-worker</text>
  <rect x="160" y="42" width="467.4" height="18" fill="#61DAFB" rx="3" opacity="0.85"/>
  <text x="633.4358974359161" y="55" fill="#cbd5ff" font-weight="500">182.3 ms</text>
  ...
  <text x="636" y="87" fill="#cbd5ff" font-weight="500">183.3 ms</text>
</svg>
...
| --- | --- | --- | --- |
| medium-1500x1000.jpg | `webcodecs-worker` | 48.4 ms | **0.72×** |
| large-4000x3000.jpg | `webcodecs-worker` | 182.3 ms | **0.93×** |
| medium-1500x1000.jpg | `offscreen-worker` | 48.2 ms | **0.73×** |
| large-4000x3000.jpg | `offscreen-worker` | 183.3 ms | **0.93×** |
| medium-1500x1000.jpg | `canvas-main` | 35.0 ms | **1.00×** |
| large-4000x3000.jpg | `canvas-main` | 170.1 ms | **1.00×** |
...
| canvas-main baseline (no sharpen) | `canvas-main` | 34.7 ms | 448.8 KB | 0.81× (-8.3 ms) | 0.0% |
| canvas-main + sharpen 0.3 | `canvas-main` | 55.2 ms | 438.8 KB | 1.28× (+12.2 ms) | 2.2% |
| WebP (q0.85) | `webcodecs-worker` | 230.1 ms | 394.4 KB | 5.35× (+187.1 ms) | 12.1% |
| WebP + qualityBoost | `webcodecs-worker` | 261.9 ms | 656.8 KB | 6.09× (+218.9 ms) | -46.4% |
| maxSizeMB: 0.4 (target-size mode) | `webcodecs-worker` | 199.5 ms | 404.3 KB | 4.64× (+156.5 ms) | 9.9% |
...
| Scenario | Path | Median | Output | vs baseline (time) | vs baseline (size) |
| --- | --- | --- | --- | --- | --- |
| Baseline (q0.85, ≤2048px, cascade) | `webcodecs-worker` | 182.9 ms | 414.2 KB | — | — |
| canvas-main baseline (no sharpen) | `canvas-main` | 171.1 ms | 414.2 KB | 0.94× (-11.8 ms) | 0.0% |
| canvas-main + sharpen 0.3 | `canvas-main` | 255.2 ms | 414.2 KB | 1.40× (+72.3 ms) | 0.0% |
| WebP (q0.85) | `webcodecs-worker` | 513.1 ms | 396.7 KB | 2.81× (+330.2 ms) | 4.2% |
| WebP + qualityBoost | `webcodecs-worker` | 592.6 ms | 885.4 KB | 3.24× (+409.7 ms) | -113.8% |
| maxSizeMB: 0.4 (target-size mode) | `webcodecs-worker` | 387.4 ms | 408.4 KB | 2.12× (+204.5 ms) | 1.4% |
...
| Run | Path | Time | Ratio |
| --- | --- | --- | --- |
| 1 | `webcodecs-worker` | 42.8 ms | 18.8% |
| 2 | `webcodecs-worker` | 43.5 ms | 18.8% |
| 3 | `webcodecs-worker` | 42.9 ms | 18.8% |
| 4 | `webcodecs-worker` | 43.7 ms | 18.8% |
| 5 | `webcodecs-worker` | 43.3 ms | 18.8% |
| 6 | `webcodecs-worker` | 49.1 ms | 18.8% |
| 7 | `webcodecs-worker` | 49.1 ms | 18.8% |
| 8 | `webcodecs-worker` | 49.2 ms | 18.8% |
| 9 | `webcodecs-worker` | 48.4 ms | 18.8% |
| 10 | `webcodecs-worker` | 49.3 ms | 18.8% |

#### large-4000x3000.jpg

| Run | Path | Time | Ratio |
| --- | --- | --- | --- |
| 1 | `webcodecs-worker` | 198.9 ms | 90.2% |
| 2 | `webcodecs-worker` | 182.3 ms | 90.2% |
| 3 | `webcodecs-worker` | 181.9 ms | 90.2% |
| 4 | `webcodecs-worker` | 181.0 ms | 90.2% |
| 5 | `webcodecs-worker` | 181.6 ms | 90.2% |
| 6 | `webcodecs-worker` | 182.2 ms | 90.2% |
| 7 | `webcodecs-worker` | 184.3 ms | 90.2% |
| 8 | `webcodecs-worker` | 180.8 ms | 90.2% |
| 9 | `webcodecs-worker` | 182.8 ms | 90.2% |
| 10 | `webcodecs-worker` | 182.6 ms | 90.2% |
...
| Run | Path | Time | Ratio |
| --- | --- | --- | --- |
| 1 | `offscreen-worker` | 42.8 ms | 18.8% |
| 2 | `offscreen-worker` | 43.0 ms | 18.8% |
| 3 | `offscreen-worker` | 43.2 ms | 18.8% |
| 4 | `offscreen-worker` | 47.0 ms | 18.8% |
| 5 | `offscreen-worker` | 50.3 ms | 18.8% |
| 6 | `offscreen-worker` | 46.3 ms | 18.8% |
| 7 | `offscreen-worker` | 48.4 ms | 18.8% |
| 8 | `offscreen-worker` | 49.2 ms | 18.8% |
| 9 | `offscreen-worker` | 48.2 ms | 18.8% |
| 10 | `offscreen-worker` | 48.9 ms | 18.8% |

#### large-4000x3000.jpg

| Run | Path | Time | Ratio |
| --- | --- | --- | --- |
| 1 | `offscreen-worker` | 201.5 ms | 90.2% |
| 2 | `offscreen-worker` | 183.3 ms | 90.2% |
| 3 | `offscreen-worker` | 184.7 ms | 90.2% |
| 4 | `offscreen-worker` | 182.2 ms | 90.2% |
| 5 | `offscreen-worker` | 186.3 ms | 90.2% |
| 6 | `offscreen-worker` | 181.8 ms | 90.2% |
| 7 | `offscreen-worker` | 182.4 ms | 90.2% |
| 8 | `offscreen-worker` | 182.4 ms | 90.2% |
| 9 | `offscreen-worker` | 182.1 ms | 90.2% |
| 10 | `offscreen-worker` | 183.5 ms | 90.2% |
...
| Run | Path | Time | Ratio |
| --- | --- | --- | --- |
| 1 | `canvas-main` | 34.5 ms | 18.8% |
| 2 | `canvas-main` | 35.7 ms | 18.8% |
| 3 | `canvas-main` | 34.6 ms | 18.8% |
| 4 | `canvas-main` | 35.1 ms | 18.8% |
| 5 | `canvas-main` | 35.0 ms | 18.8% |
| 6 | `canvas-main` | 34.7 ms | 18.8% |
| 7 | `canvas-main` | 34.3 ms | 18.8% |
| 8 | `canvas-main` | 35.0 ms | 18.8% |
| 9 | `canvas-main` | 34.6 ms | 18.8% |
...
| 2 | `canvas-main` | 170.4 ms | 90.2% |
| 3 | `canvas-main` | 170.1 ms | 90.2% |
| 4 | `canvas-main` | 175.5 ms | 90.2% |
| 5 | `canvas-main` | 167.8 ms | 90.2% |
| 6 | `canvas-main` | 167.8 ms | 90.2% |
| 7 | `canvas-main` | 170.8 ms | 90.2% |
| 8 | `canvas-main` | 168.1 ms | 90.2% |
| 9 | `canvas-main` | 169.7 ms | 90.2% |
| 10 | `canvas-main` | 173.2 ms | 90.2% |
